...
Feature: Earn scenarios for SPOS PURCHASE TRANSACTION for different tenders and
  ...
  Scenario: Earn scenario for SPOS PURCHASE TRANSACTION with THIRD PARTY and excluded dept
    ...
    And I Perform IRN Transaction for TFS enrollment with one line item
    ...
    #Perform TFS Transcation with THIRD PARTY as Tender for excluded dept
    And I Perform IRN Transaction for TFS Enrollment with excluded dept and THIRD PARTY tender
  ...
  Scenario: Earn scenario for SPOS PURCHASE TRANSACTION with THIRD PARTY and included dept
    ...
    Then I LookUp Customer by Account Number and Verify TFS Enrollment
      | requestMethod | requestFile                          | responseFile                       |
      | POST          | Request_cusLookupByAccountNumber.xml | Response_cusLookup_TFSEnrolled.xml |
    #Perform TFS Transcation with THIRD PARTY as Tender for included dept
    And I Perform IRN Transaction for TFS Enrollment with included dept and THIRD PARTY tender
      | TenderType | DynamicDepts | PointsExpected | requestMethod | requestFile                                      | responseFile                       |
  ...
  Scenario: Earn scenario for SPOS PURCHASE TRANSACTION with cash tender and excluded dept
    ...
    Then I LookUp Customer by Account Number and Verify TFS Enrollment
      | requestMethod | requestFile                          | responseFile                       |
      | POST          | Request_cusLookupByAccountNumber.xml | Response_cusLookup_TFSEnrolled.xml |
    #Perform TFS Transcation with CASH as Tender for excluded dept
    And I Perform IRN Transaction for TFS Enrollment with excluded item and cash tender
      | TenderType | DynamicDepts | PointsExpected | requestMethod | requestFile                                      | responseFile                       |
  ...
  Scenario: Earn scenario for SPOS PURCHASE TRANSACTION with cash tender and included dept
    ...
    Then I LookUp Customer by Account Number and Verify TFS Enrollment
      | requestMethod | requestFile                          | responseFile                       |
      | POST          | Request_cusLookupByAccountNumber.xml | Response_cusLookup_TFSEnrolled.xml |
    #Perform TFS Transcation with CASH as Tender for included dept
    And I Perform IRN Transaction for TFS Enrollment with included dept and cash tender
      | TenderType | DynamicDepts | PointsExpected | requestMethod | requestFile                                      | responseFile                       |
  ...
  Scenario: Earn for SPOS PURCHASE TRANSACTION with GIFT CARD and excluded dept
    ...
    Then I LookUp Customer by Account Number and Verify TFS Enrollment
      | requestMethod | requestFile                          | responseFile                       |
      | POST          | Request_cusLookupByAccountNumber.xml | Response_cusLookup_TFSEnrolled.xml |
    #Perform TFS Transcation with GIFT CARD as Tender for excluded dept
    And I Perform IRN Transaction for TFS Enrollment with excluded dept and GIFT CARD tender
      | TenderType | DynamicDepts | PointsExpected | requestMethod | requestFile                                      | responseFile                       |
  ...
  Scenario: Earn for SPOS PURCHASE TRANSACTION with GIFT CARD and included dept
    ...
    Then I LookUp Customer by Account Number and Verify TFS Enrollment
      | requestMethod | requestFile                          | responseFile                       |
      | POST          | Request_cusLookupByAccountNumber.xml | Response_cusLookup_TFSEnrolled.xml |
    #Perform TFS Transcation with GIFT CARD as Tender for included dept
    And I Perform IRN Transaction for TFS Enrollment with included dept and GIFT CARD tender
      | TenderType | DynamicDepts | PointsExpected | requestMethod | requestFile                                      | responseFile                       |
  ...
  Scenario: Earn for SPOS PURCHASE TRANSACTION with PROP tender, cash tender and excluded dept
    ...
    Then I LookUp Customer by Account Number
      ...
      | POST          | Request_cusLookupByAccountNumber.xml | Response_cusLookup_TFSDisEnrolled.xml |
    Then I wait for 4000 millis
    #Perform TFS Transcation with PROP, CASH as tenders for excluded dept
    And I Perform PROP Transaction for TFS enrollment with excluded dept and CASH tender
  ...
  Scenario: Earn for SPOS PURCHASE TRANSACTION with PROP tender, cash tender and included dept
    ...
    Then I LookUp Customer by Account Number
      ...
      | POST          | Request_cusLookupByAccountNumber.xml | Response_cusLookup_TFSDisEnrolled.xml |
    Then I wait for 4000 millis
    #Perform TFS Transcation with PROP, CASH as tenders for included dept
    And I Perform PROP Transaction for TFS enrollment with included item and CASH tender
      | TenderType | DynamicDepts | PointsExpected | requestMethod | requestFile                                     | responseFile                                              |
      |         26 |      358,456 |           5.24 | POST          | Request_LTYTransaction_EnrollWithTwoTenders.xml | Response_LTYTransaction_WithNonexcludedDept_TFSEnroll.xml |
    Then I Validate TFS enrollment in transaction response
  ...
  Scenario: Earn for SPOS PURCHASE TRANSACTION with IRN tender, GC with included dept scenario
    ...
    Then I LookUp Customer by Account Number
      ...
      | POST          | Request_cusLookupByAccountNumber.xml | Response_cusLookup_TFSDisEnrolled.xml |
    Then I wait for 4000 millis
    #Perform TFS Transcation with IRN, GIFT CARD as tenders for included dept
    And I Perform IRN Transaction for TFS enrollment with included dept and GIFT CARD
      | TenderType | DynamicDepts | PointsExpected | requestMethod | requestFile                                     | responseFile                                              |
      |         47 |      358,456 |           5.24 | POST          | Request_LTYTransaction_EnrollWithTwoTenders.xml | Response_LTYTransaction_WithNonexcludedDept_TFSEnroll.xml |
    Then I Validate TFS enrollment in transaction response
  ...
  Scenario: Earn for SPOS PURCHASE TRANSACTION with IRN tender, GC with excluded dept scenario
    ...
    Then I LookUp Customer by Account Number
      ...
      | POST          | Request_cusLookupByAccountNumber.xml | Response_cusLookup_TFSDisEnrolled.xml |
    Then I wait for 4000 millis
    #Perform TFS Transcation with IRN, GIFT CARD as tenders for excluded dept
    And I Perform IRN Transaction for TFS enrollment with excluded dept and GIFT CARD
      | TenderType | DynamicDepts | requestMethod | requestFile                                     | responseFile                                       |
  ...
  Scenario: Earn for SPOS PURCHASE TRANSACTION with PROP tender backstage scenario and included dept
    ...
    Then I LookUp Customer by Account Number
      ...
      | POST          | Request_cusLookupByAccountNumber.xml | Response_cusLookup_TFSDisEnrolled.xml |
    Then I wait for 4000 millis
    #Perform TFS Transcation with PROP tender backstage scenario for included dept
    And I Perform PROP Transaction for TFS enrollment with included dept
  ...
  Scenario: Earn for SPOS PURCHASE TRANSACTION with PROP tender backstage scenario and excluded dept
    ...
    Then I LookUp Customer by Account Number
      ...
      | POST          | Request_cusLookupByAccountNumber.xml | Response_cusLookup_TFSDisEnrolled.xml |
    Then I wait for 4000 millis
    #Perform TFS Transcation with PROP tender backstage scenario for excluded dept
    And I Perform PROP Transaction for TFS enrollment with excluded dept
      | DynamicClass | DynamicDepts | requestMethod | requestFile                           | responseFile                                       |
